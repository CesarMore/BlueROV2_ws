...
float x1, x2, T1, vnort, vest, XX, XXdes_pf, XXdes_f, VelNorte;
...
uint16_t uyaw=0, yyaw=0, usyaw=0, xposxx=0, uxx=0, usxx=0;
		...
		//################# CICLO DEL CONTROLADOR #################
	  	if(controles == true)
	  	{
			...
			*/			
    		...
    		ofstream f1("Figura1.csv");
    		for (int ii = 0; ii < tiempo; ii++) 
			{
        		f1 << tiempo << "," << posxx << endl;
    		}
    		f1.close();

			ofstream f2("Figura2.csv");
    		for (int ii = 0; ii < tiempo; ii++) 
			{

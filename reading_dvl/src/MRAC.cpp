...
float Gammayaw = 10, Gammar_yaw =10, Gamma_Theta_yaw=10, Kyaw_hat=0, Kr_hat_yaw=0, Theta_hat_yaw=0;
float Kx_hat=0, Kr_hat_x=0, Theta_hat_x=0, Ky_hat=0, Kr_hat_y=0, Theta_hat_y=0;
double Xdeseada, Ydeseada, Xactual, Yactual, YawDes, YawActual;
  	...
  	{	
		...
		//################# CICLO DEL CONTROLADOR #################
	  	if(controles == true)
	  	{
			...
			Tf = 30;	// Tiempo de trayectoria cuadrada
			...
			*/
			...
			//######################       MRAC       ###################
			//##################### Control de Yaw ###################
			kpyaw=2.5, kiyaw=0.01, kdyaw=0.5;
			yawDes = -145.0;
			yawDes_p = 0.0;
			...
			//yyaw = -( kpyaw*eyaw + kiyaw*eyaw_i + kdyaw*eyaw_p );							

			/*Gammayaw = 10, Gammar_yaw =10, Gamma_Theta_yaw=10;
			Kyaw_hat = Kyaw_hat + 0.05*( -Gammayaw * posyaw * eyaw * 1 * 1 );
    		Kr_hat_yaw = Kr_hat_yaw + 0.05*( -Gammar_yaw * yawDes * eyaw * 1 * 1 );
    		Theta_hat_yaw = Theta_hat_yaw + 0.05*( Gamma_Theta_yaw * posyaw_p * eyaw * 1 * 1 );*/
			
			Kyaw_hat = Kyaw_hat - 0.05*( 5*posyaw*eyaw + 7.143*posyaw*eyaw_p);
    		Kr_hat_yaw = Kr_hat_yaw - 0.05*( 5*posyaw*eyaw + 7.143*posyaw*eyaw_p );
    		Theta_hat_yaw = Theta_hat_yaw + 0.05*( 5*posyaw_p*eyaw + 7.143*posyaw_p*eyaw_p );

			yyaw = -( Kyaw_hat*posyaw + Kr_hat_yaw*yawDes -  Theta_hat_yaw*posyaw_p );							
			...
			Kx_hat = Kx_hat - 0.05*( 50*posx*exx + 71.43*posx*exx_p);
    		Kr_hat_x = Kr_hat_x - 0.05*( 50*posx*exx + 71.43*posx*exx_p );
    		Theta_hat_x = Theta_hat_x + 0.05*( 50*vnort*exx + 71.43*vnort*exx_p );

			ux_B = -( Kx_hat*posx + Kr_hat_x*posx - Theta_hat_x*vnort );							
			...
			// Matriz de rotación
			ux_I = -( Kx_hat*posx + Kr_hat_x*posx - Theta_hat_x*vnort );
		...
		ROS_INFO_STREAM("DatosPos_X: " << tiempo2  << "\t" << exx  << "\t" << ux_B << "\t" << usxx << "\t" << posxx << "\t" << xd << "\t" << tr[5] << endl);
		//ROS_INFO_STREAM("DatosPos_Y: " << tiempo2  << "\t" << eyy  << "\t" << uy_B << "\t" << usyy << "\t" << posy << "\t" << yd << "\t" << tr[6] << endl);
		
		ROS_INFO_STREAM("DatosPos_Yaw: " << tiempo2  << "\t" << eyaw  << "\t" << yyaw << "\t" << usyaw << "\t" << posyaw << "\t" << Kyaw_hat << "\t" << Kr_hat_yaw << "\t" << Theta_hat_yaw << endl);

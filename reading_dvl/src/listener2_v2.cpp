...
int paro=0,  bajar=0,  armar=0,   control_pos= 0,   desarmar=0, controles=0, controles2=0;
  	...
  	while (ros::ok())
  	{
		...
		//################# CICLO DEL CONTROLADOR #################
	  	if(controles == true)
	  	{
			...
			yawc.data=usyaw;       // asigna valor a data
			pubYaw.publish(yawc);  //publica yawc

			eyaw_ant = eyaw;  //guarda variables anteriores
    	...
    	for (int i = 1; i < tiempo; i++) {
        	posx1[i] = posx1[i-1] + Inc_dpx[i][7];
        	posy2[i] = posy2[i-1] + Inc_dpy[i][8];
    	}
		*/
		// Calcula X y Y usando la regla del trapezoide
		x1[0] = 0;
		x2[0] = 0;
    	...
    	for (int i = 0; i < tiempo-1; i++)
		{
			vnort[i] = tr[6];
			vest[i] = tr[7];
        	x1[i+1] = x1[i] + (T1[i+1]-T1[i])*(vnort[i]);
        	x2[i+1] = x2[i] + (T1[i+1]-T1[i])*(vest[i]);
    	}
		...
		//vnort[] = tr[6];
	  	if(controles == true)
	  	{
			...
			//################# INCIA EL CONTROLADOR ##################
			//########### Control de de trayectoria en eje X ##########
			exx = (x1 - XXdes_f);  									//Error
			exx_p = (vnort  - XXdes_pf);					//Error p

			uposx = Kp_x*exx + Kd_x*exx_p;		//PD
	  	}
	  	else
	  	{
			// MODO DE OPERACION MANUAL
    	...
    	//ROS_INFO_STREAM("Datos2: " << s <<endl);
     	...
     	//ROS_INFO_STREAM("DVL1: " << dvl1 <<endl);
      	...
      	//<< tr[4] << "\t" << tr[5] << "\t" << "vel_norte="<< tr[6] << "\t" << "vel_este="<< tr[7] << endl ) ;
      	//ROS_INFO_STREAM("Datos1: " << "vel_norte="<< tr[6] << "\t" << "vel_este="<< tr[7] << endl ) ;
	  	...
	  	//myfile << std::setprecision(5) << tiempo << "roll\t"   << tr[0]  << "pitch\t" << tr[1]  << "yaw\t" << tr[2]
      	//<< "vel_norte\t" << tr[6]  << "vel_este\t" << tr[7] << "roll\t"
      	//<< posroll << "pitch\t" << pospitch  << "yaw\t" << posyaw << "\n";

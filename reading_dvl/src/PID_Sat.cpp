...
int satxx =0, satyy=0, satz=0, satyaw=0;
  	...
  	{	
		...
		//################# CICLO DEL CONTROLADOR #################
	  	if(controles == true)
	  	{
			...
			//kpyaw=0.5, kiyaw=0.01, kdyaw=0.5;  
			kpyaw=1, kiyaw=0.01, kdyaw=1;  
			//yawDes = -145.0;
			//yawDes_p = 0.0;
			...
			satkpyaw = satK(eyaw,20,kpyaw,0.5);
			satkdyaw = satK(eyaw_p,20,kdyaw,0.5);
			satkiyaw = satK(eyaw_i,20,kiyaw,0.5);
			
			yyaw = -(satkpyaw*eyaw + satkdyaw*eyaw_p + satkiyaw*eyaw_i);
			...
			Kp_x=7, Ki_x=0.01, Kd_x=0.3;  	
			Kp_y=7, Ki_y=0.01, Kd_y=0.2; 
			...
			satkpx = satK(exx,20,Kp_x,0.5);
			satkdx = satK(exx_p,20,Kd_x,0.5);
			satkix = satK(exx_i,20,Ki_x,0.5);
			
			ux_B = (satkpx*exx + satkdx*exx_p + satkix*exx_i);
			...
			satkpy = satK(eyy,20,Kp_y,0.5);
			satkdy = satK(eyy_p,20,Kd_y,0.5);
			satkiy = satK(eyy_i,20,Ki_y,0.5);
			
			uy_B = -(satkpy*eyy + satkdy*eyy_p + satkiy*eyy_i);

...
void presCallback(const sensor_msgs::FluidPressure::ConstPtr & msg) {
    //ROS_INFO( "Press: %.3f,Diff_press:%.3f,", msg->fluid_pressure, msg->variance);
	 ...
	 //z_baro = (fluid_press/(Rf*g)*100-7.39);  fondo 43 cm
	 z_baro=fluid_press-726;
	...
	while (ros::ok())
  	{	
		...
		//################# CICLO DEL CONTROLADOR #################
	  	if(controles == true)
	  	{
			...
			*/
			...
			varphi_yaw = eyaw/chi_yaw;
			
			aux1_yaw =    1/(varphi_yaw+delta_inf_yaw) - 1 / (varphi_yaw - delta_sup_yaw) ;
			
			rho_yaw = (1 / (2*chi_yaw) ) * aux1_yaw;
			
			j_yaw  = (varphi_yaw + delta_inf_yaw)/(delta_sup_yaw-varphi_yaw) ;
			
			z1_yaw = log( j_yaw)/2;
			
			z1_p_yaw = rho_yaw * (eyaw_p - (eyaw * chi_p_yaw) /chi_yaw);
			
			mu_yaw = -(2*chi_p_yaw)/(2*chi_yaw);
			
			eta_yaw = 1/ (2*chi_yaw);
			
			varphi_p_yaw = (eyaw_p * chi_yaw - eyaw * chi_p_yaw ) / (chi_yaw*chi_yaw);
			
							
			rho_p_yaw = (0.5* varphi_p_yaw / chi_yaw) * ( 1/((varphi_yaw - delta_sup_yaw)*(varphi_yaw - delta_sup_yaw)) - 1/ ((varphi_yaw + delta_inf_yaw)*(varphi_yaw + delta_inf_yaw)) ) - (chi_p_yaw/(chi_yaw*chi_yaw)) * aux1_yaw;
			
		
			alpha_yaw = rho_p_yaw * ( eyaw_p - (eyaw * chi_p_yaw )/chi_yaw );
			
			beta_yaw = (-yawDes_pp - eyaw_p * chi_p_yaw)/chi_yaw - (eyaw * chi_pp_yaw*chi_yaw)/(chi_yaw*chi_yaw) + eyaw * (chi_p_yaw*chi_p_yaw)/(chi_yaw*chi_yaw);

			float kpyaw=3.50, kiyaw=0, kdyaw=0.50; 									//Ganancias yaw
		
			//smc ganancias  superficie
			float s_yaw=0, delta_yaw=1, k1_yaw=0, k2_yaw=0;
			float alpha1_yaw=0.65, beta1_yaw=0.55, m1_yaw =1.1, m2_yaw =0.75 ;  //Real ppcfft
										
			k1_yaw= ((m1_yaw+1)/2)+((m1_yaw-1)/2)*sgn(abs(posyaw)-1);
			k2_yaw= ((m2_yaw+1)/2)+((1-m2_yaw)/2)*sgn(abs(posyaw)-1);
					
			s_yaw = z1_p_yaw + alpha1_yaw* pow(abs(z1_yaw),k1_yaw)*sgn(z1_yaw) + beta1_yaw*pow(abs(z1_yaw),k2_yaw)*sgn(z1_yaw);
			
			//smc ganancias
			float   alpha2_yaw=0.60, beta2_yaw=0.50, gamma1_yaw=0, gamma2_yaw=0, k3_yaw = 0.3; //real ppcfft
			
			float n1_yaw =1.1, n2_yaw =0.75;
			
			gamma1_yaw= ((n1_yaw+1)/2)+((n1_yaw-1)/2)*sgn(abs(s_yaw)-1);
			gamma2_yaw= ((n2_yaw+1)/2)+((1-n2_yaw)/2)*sgn(abs(s_yaw)-1);
			//pow(

			yyaw=-(alpha_yaw - yawDes_pp + alpha1_yaw*k1_yaw*pow(abs(z1_yaw),(k1_yaw-1))*veaz + beta1_yaw*k2_yaw*pow(abs(z1_yaw),(k2_yaw-1))*veaz + alpha2_yaw*pow(abs(s_yaw),gamma1_yaw)*sgn(s_yaw) + beta2_yaw*pow(abs(s_yaw),gamma2_yaw)*sgn(s_yaw)+k3_yaw*sgn(s_yaw))/rho_yaw + beta_yaw ;							//PPC-FFNTSM

			uyaw=offset(yyaw);     													//aplica offset de  motores en la señal del controlador con u negativa
			
			usyaw=saturayaw((int)uyaw);											    //Satura la señal de pwm entre 1400 a 1600
			
			yawc.data=usyaw; //cambiar a usyaw
			pubYaw.publish(yawc);
		...
		////////////////////////////////////////Control de X
			...
			k1_x= ((m1_x+1)/2)+((m1_x-1)/2)*sgn(abs(posx)-1);
			k2_x= ((m2_x+1)/2)+((1-m2_x)/2)*sgn(abs(posx)-1);
			...
			//smc ganancias
			float   alpha2_x=0.6, beta2_x=0.50, gamma1_x=0, gamma2_x=0, k3_x = 0.3; //real ppcfft funciono bien 18_may_23 ref filtrada
		...
		////////////////////////////////////////Control de Y
			...
			k1_y= ((m1_y+1)/2)+((m1_y-1)/2)*sgn(abs(posy)-1);
			k2_y= ((m2_y+1)/2)+((1-m2_y)/2)*sgn(abs(posy)-1);
		...
		ROS_INFO_STREAM("DatosPos_X: " << tiempo2  << "\t" << exx  << "\t" << ux_B << "\t" << usxx << "\t" << posxx << "\t" << usyaw << "\t" << posyaw << "\t" << tr[5]  << endl);
		ROS_INFO_STREAM("DatosPos_Y: " << tiempo2  << "\t" << eyy  << "\t" << uy_B << "\t" << usyy << "\t" << posyy << "\t" << usyaw << "\t" << posyaw << "\t" << tr[6] << endl);
